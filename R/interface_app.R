...
#' @examples
#' \dontrun{
#' # Create the interface app
#' app <- create_interface_app()
#' shiny::runApp(app)
#' }
#' @export
#' @author Jared Andrews
create_interface_app <- function() {
    # Load pre-computed UI choices for faster startup
    # These choices are generated by inst/app/generate_ui_choices.R
    ui_choices_path <- system.file("app", "ui_choices.rds", package = "cellNexus")
    if (file.exists(ui_choices_path)) {
        all_choices <- readRDS(ui_choices_path)
    } else {
        # Fall back to empty choices if pre-computed file not found
        warning("Pre-computed UI choices not found. Run inst/app/generate_ui_choices.R to generate them.")
        all_choices <- list()
    }
